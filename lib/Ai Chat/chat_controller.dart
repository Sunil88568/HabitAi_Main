...
class AICoachController extends GetxController
  ...
  final Rxn<String> lastDetectedIssue = Rxn<String>();
  final Rxn<String> suggestedHabitType = Rxn<String>();
  ...
  void _sendAIMessage(String message) async {
    ...
    final coachResponse = _getContextualResponse(message);
  ...
  CoachResponse? _getContextualResponse(String userMessage) {
    final msg = userMessage.toLowerCase();
    
    // Stress & Mental Health
    if (msg.contains('stress') || msg.contains('anxious') || msg.contains('anxiety') || msg.contains('overwhelm') || 
        msg.contains('panic') || msg.contains('worry') || msg.contains('nervous') || msg.contains('tense') ||
        msg.contains('burnout') || msg.contains('mental health') || msg.contains('depressed')) {
      return CoachResponse(
        coachAction: 'ADVICE',
        message: 'I understand stress can be overwhelming. Would you like to try a 5-minute daily meditation to help manage it?',
        nextQuestions: ['Yes, create meditation habit', 'No, suggest something else'],
        tone: 'supportive',
        confidence: 0.9,
      );
    }
    ...
    // Energy & Fatigue
    if (msg.contains('energy') || msg.contains('tired') || msg.contains('fatigue') || msg.contains('exhausted') ||
        msg.contains('drained') || msg.contains('sluggish') || msg.contains('lethargic') || msg.contains('weak') ||
        msg.contains('no motivation') || msg.contains('lazy') || msg.contains('sleepy during day')) {
      return CoachResponse(
        coachAction: 'ADVICE',
        message: 'Low energy can be tough. A simple morning stretch routine might help boost your energy. Shall we create that habit?',
        nextQuestions: ['Yes, create stretch habit', 'No, suggest something else'],
        tone: 'supportive',
        confidence: 0.9,
      );
    }
    ...
    // Sleep Issues
    if (msg.contains('sleep') || msg.contains('cant sleep') || msg.contains('cannot sleep') || 
        msg.contains('insomnia') || msg.contains('not able to sleep') || msg.contains('trouble sleeping') ||
        msg.contains('hard to sleep') || msg.contains('difficulty sleeping') || msg.contains('restless') ||
        msg.contains('wake up') || msg.contains('nightmares') || msg.contains('tossing turning')) {
      return CoachResponse(
        coachAction: 'ADVICE',
        message: 'Sleep issues can be challenging. A "no screens before bed" habit could help. Would you like to try this?',
        nextQuestions: ['Yes, create sleep habit', 'No, suggest something else'],
        tone: 'supportive',
        confidence: 0.9,
      );
    }
    ...
    // Productivity & Focus
    if (msg.contains('productive') || msg.contains('focus') || msg.contains('distracted') || msg.contains('procrastinate') ||
        msg.contains('concentration') || msg.contains('attention') || msg.contains('organize') || msg.contains('time management') ||
        msg.contains('get things done') || msg.contains('accomplish') || msg.contains('efficiency')) {
      return CoachResponse(
        coachAction: 'ADVICE',
        message: 'Productivity struggles are common. A "plan tomorrow tonight" habit could help you stay focused. Would you like to try this?',
        nextQuestions: ['Yes, create planning habit', 'No, suggest something else'],
        tone: 'supportive',
        confidence: 0.9,
      );
    }
    ...
    // Health & Fitness
    if (msg.contains('health') || msg.contains('fitness') || msg.contains('exercise') || msg.contains('weight') ||
        msg.contains('out of shape') || msg.contains('sedentary') || msg.contains('inactive') || msg.contains('movement') ||
        msg.contains('physical activity') || msg.contains('body') || msg.contains('strength')) {
      return CoachResponse(
        coachAction: 'ADVICE',
        message: 'Health is so important. A simple "take stairs instead of elevator" habit could be a great start. Shall we create this?',
        nextQuestions: ['Yes, create health habit', 'No, suggest something else'],
        tone: 'supportive',
        confidence: 0.9,
      );
    }
    ...
    // Hydration & Nutrition
    if (msg.contains('water') || msg.contains('hydrat') || msg.contains('drink') || msg.contains('thirsty') ||
        msg.contains('dehydrated') || msg.contains('nutrition') || msg.contains('eating') || msg.contains('diet') ||
        msg.contains('hungry') || msg.contains('food') || msg.contains('meal')) {
      return CoachResponse(
        coachAction: 'ADVICE',
        message: 'Staying hydrated is crucial! A "drink water first thing in morning" habit could help. Would you like to try this?',
        nextQuestions: ['Yes, create water habit', 'No, suggest something else'],
        tone: 'supportive',
        confidence: 0.9,
      );
    }
    ...
    if (msg.contains('lonely') || msg.contains('social') || msg.contains('friends') || msg.contains('relationship') ||
        msg.contains('isolated') || msg.contains('connect') || msg.contains('communication') || msg.contains('family')) {
      return CoachResponse(
        coachAction: 'ADVICE',
        message: 'Social connections are vital. A "call one friend daily" habit could help strengthen relationships. Shall we create this?',
        nextQuestions: ['Yes, create social habit', 'No, suggest something else'],
        tone: 'supportive',
        confidence: 0.9,
      );
    }
    ...
    // Learning & Growth
    if (msg.contains('learn') || msg.contains('study') || msg.contains('skill') || msg.contains('knowledge') ||
        msg.contains('read') || msg.contains('book') || msg.contains('education') || msg.contains('improve') ||
        msg.contains('develop') || msg.contains('grow')) {
      return CoachResponse(
        coachAction: 'ADVICE',
        message: 'Learning is wonderful! A "read 10 pages daily" habit could help you grow consistently. Would you like to try this?',
        nextQuestions: ['Yes, create reading habit', 'No, suggest something else'],
        tone: 'supportive',
        confidence: 0.9,
      );
    }
    
    // Check for agreement to create habits
    if (msg.contains('yes, create meditation')) return _createHabitForType('meditation');
    if (msg.contains('yes, create stretch')) return _createHabitForType('stretch');
    if (msg.contains('yes, create planning')) return _createHabitForType('planning');
    if (msg.contains('yes, create sleep')) return _createHabitForType('sleep');
    if (msg.contains('yes, create health')) return _createHabitForType('health');
    if (msg.contains('yes, create water')) return _createHabitForType('water');
    if (msg.contains('yes, create social')) return _createHabitForType('social');
    if (msg.contains('yes, create reading')) return _createHabitForType('reading');
    
    // General agreement
    if (msg.contains('yes') && (msg.contains('create') || msg.contains('habit'))) {
      return _createHabitForType('walk');
    }
    
    // Default response
    return CoachResponse(
      coachAction: 'ADVICE',
      message: 'I hear you. What specific challenge would you like to work on? I can help you create a habit to address it.',
      nextQuestions: ['I feel stressed', 'I lack energy', 'I want to be more productive'],
      tone: 'supportive',
      confidence: 0.8,
    );
  }
  
  CoachResponse _createHabitForType(String habitType) {
    switch (habitType) {
      case 'meditation':
        return CoachResponse(
          coachAction: 'CREATE_HABIT',
          message: 'Perfect! Creating your meditation habit now.',
          nextQuestions: ['Create another habit', 'I\'m done for now'],
      ...
      case 'sleep':
        return CoachResponse(
          coachAction: 'CREATE_HABIT',
          message: 'Perfect! Creating your sleep hygiene habit now.',
          nextQuestions: ['Create another habit', 'I\'m done for now'],
          habitTemplate: HabitTemplate(
            title: 'No Screens Before Bed',
            subtitle: 'Better sleep quality',
            cadence: HabitCadence(days: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], times: ['21:00']),
            category: 'health',
          ),
          tone: 'celebratory',
          confidence: 1.0,
      ...
      case 'planning':
        return CoachResponse(
          coachAction: 'CREATE_HABIT',
          message: 'Great! Creating your planning habit now.',
          nextQuestions: ['Create another habit', 'I\'m done for now'],
          habitTemplate: HabitTemplate(
            title: 'Plan Tomorrow Tonight',
            subtitle: 'Better productivity',
            cadence: HabitCadence(days: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], times: ['20:00']),
            category: 'work',
          ),
          tone: 'celebratory',
          confidence: 1.0,
      ...
      case 'health':
        return CoachResponse(
          coachAction: 'CREATE_HABIT',
          message: 'Excellent! Creating your health habit now.',
          nextQuestions: ['Create another habit', 'I\'m done for now'],
          habitTemplate: HabitTemplate(
            title: 'Take Stairs',
            subtitle: 'Daily movement',
            cadence: HabitCadence(days: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], times: ['12:00']),
            category: 'health',
          ),
          tone: 'celebratory',
          confidence: 1.0,
      ...
      case 'water':
        return CoachResponse(
          coachAction: 'CREATE_HABIT',
          message: 'Perfect! Creating your hydration habit now.',
          nextQuestions: ['Create another habit', 'I\'m done for now'],
          habitTemplate: HabitTemplate(
            title: 'Morning Water',
            subtitle: 'Start day hydrated',
            cadence: HabitCadence(days: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], times: ['07:00']),
            category: 'health',
          ),
          tone: 'celebratory',
      ...
      case 'social':
        return CoachResponse(
          coachAction: 'CREATE_HABIT',
          message: 'Great! Creating your social connection habit now.',
          nextQuestions: ['Create another habit', 'I\'m done for now'],
          habitTemplate: HabitTemplate(
            title: 'Call One Friend',
            subtitle: 'Daily social connection',
            cadence: HabitCadence(days: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], times: ['19:00']),
      ...
      case 'reading':
        return CoachResponse(
          coachAction: 'CREATE_HABIT',
          message: 'Wonderful! Creating your reading habit now.',
          nextQuestions: ['Create another habit', 'I\'m done for now'],
          habitTemplate: HabitTemplate(
            title: 'Read 10 Pages',
      ...
      default:
        return CoachResponse(
          coachAction: 'CREATE_HABIT',
          message: 'Perfect! Creating a simple daily walk habit for you.',
          nextQuestions: ['Create another habit', 'I\'m done for now'],
          habitTemplate: HabitTemplate(
            title: '5-Minute Daily Walk',
            subtitle: 'Small step, big impact',
            cadence: HabitCadence(days: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], times: ['10:00']),
            category: 'health',
          ),
          tone: 'celebratory',
          confidence: 1.0,
